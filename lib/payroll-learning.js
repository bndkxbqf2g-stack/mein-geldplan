...
export function buildPayrollLearningSnapshot({forecast,actual,tolerance=1}={}){
  ...
  const matchingRetro=(actual?.retroPeriods||[]).find(period=>period?.month&&period.month===forecast?.reportMonth)||null;
  const componentActual=matchingRetro||actual;
  ...
  return {
    ...
    componentSource:matchingRetro?'retro':'current',
    componentSourceMonth:matchingRetro?.month||actual.month
  };
}
