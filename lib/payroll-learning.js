...
function learningComponentSource(forecast,actual,payslips=[]){
  ...
  const slips=[actual,...(Array.isArray(payslips)?payslips:[]).filter(item=>item&&item!==actual)];
  const retros=[],seen=new Set();
  for(const payslip of slips){
    for(const period of payslip?.retroPeriods||[]){
      ...
      const key=[payslip?.month,period?.month,period?.totalGross,JSON.stringify(period?.components||{})].join('|');
      if(seen.has(key))continue;
      seen.add(key);retros.push(period);
    }
  }
  if(!retros.length)return {source:actual,label:'current',month:actual?.month||null};

  const keys=Object.keys(COMPONENTS);
  const currentHasDetail=Boolean(actual?.components?.hasVariableDetail)||keys.some(key=>finiteComponent(actual,key)!=null);
  const sources=[...(currentHasDetail?[actual]:[]),...retros];
  const components={};
  ...
  return {
    source:{components},
    label:currentHasDetail?'current+retro':'retro',
    month:forecast?.reportMonth||retros[0]?.month||actual?.month||null
  };
}
