...
export function projectPendingSalary({pending,giro=0,cash=0,fixedCosts=[]}={}){
  if(!pending)return null;
  const fixed=totalFixedCosts(fixedCosts);
  return {
    fixedCosts:fixed,
...
export function applyPendingSalary({pending,transactions=[],fixedCosts=[],today=new Date()}={}){
