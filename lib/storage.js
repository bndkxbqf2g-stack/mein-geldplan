...
export function clearAllStorage(storage) {
  const store = getStorage(storage); if (!store) return false;
  try { [TX_KEY,CASH_KEY,SAVINGS_KEY,SALARY_KEY,PAYSLIP_KEY,FIXED_KEY,FIXED_OVERRIDE_KEY,PENDING_SALARY_KEY,BUDGET_ANCHOR_KEY,SCHEMA_KEY].forEach(k=>store.removeItem(k)); return true; } catch { return false; }
}
export const storageKeys = Object.freeze({
  transactions: TX_KEY, cash: CASH_KEY, savings: SAVINGS_KEY, salaryForecasts: SALARY_KEY,
  payslips: PAYSLIP_KEY, fixedCosts: FIXED_KEY, fixedCostOverrides: FIXED_OVERRIDE_KEY, pendingSalary:PENDING_SALARY_KEY,
