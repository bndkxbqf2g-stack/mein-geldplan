...
import {buildPayrollControlHistory,buildPayrollCarryovers,payrollCarryoverRegularSollLabel,payrollControlStatusLabel} from './payroll-control.js';
...
export function renderPayrollControl(){
  ...
  if(status)status.textContent=open?open+' Abrechnungsmonat(e) mit offenem Bruttoanspruch.':review?'Kontrolle enthält Positionen zur Prüfung.':'Alle vollständig prüfbaren Monate sind ausgeglichen.';
  ...
  for(const item of controls){
    ...
    for(const entry of [['Soll-Brutto',item.expectedGross],['Abgerechnet-Brutto',item.actualGross],['Brutto fehlt',item.remainingGross]]){
      ...
      if(entry[0]==='Brutto fehlt'&&Number(entry[1])>1)n.classList.add('red');
    ...
    const payoutValue=document.createElement('span');payoutValue.className='v';
    if(item.actualPayout!=null){
      payoutLabel.textContent=item.hasPriorAdjustment?'Tatsächlich ausgezahlt inkl. Nachverrechnung':'Tatsächlich ausgezahlt';
      payoutValue.textContent=value(item.actualPayout);
    }else{
      payoutLabel.textContent='Prognostizierte Auszahlung';
      payoutValue.textContent=value(item.expectedPayout);
    }
    payoutRow.append(payoutLabel,payoutValue);card.appendChild(payoutRow);

    const incoming=carryovers[item.payoutMonth]||[];
    if(incoming.length){
      const cg=document.createElement('div');cg.className='payroll-control-group';cg.textContent='Offene Nachzahlung aus Vormonat';card.appendChild(cg);
      for(const carry of incoming){
        const r=document.createElement('div');r.className='row';
        const l=document.createElement('span');l.textContent='Aus '+carry.sourceLabel;
        if(carry.gross!=null){
          const small=document.createElement('small');small.textContent=eur(carry.gross)+' brutto noch offen';l.appendChild(small);
        }
        const v=document.createElement('span');v.className='v good';v.textContent='+'+eur(carry.net);
        r.append(l,v);card.appendChild(r);
      }
      const carryTotal=incoming.reduce((sum,entry)=>sum+(Number(entry.net)||0),0);
      if(item.actualPayout==null&&item.expectedPayout!=null&&carryTotal>0.005){
        const r=document.createElement('div');r.className='row';
        const l=document.createElement('span');const b=document.createElement('b');b.textContent='Mögliche Auszahlung inkl. offener Nachzahlung';l.appendChild(b);
        const small=document.createElement('small');small.textContent=payrollCarryoverRegularSollLabel(item);l.appendChild(small);
        const v=document.createElement('span');v.className='v good';v.textContent=eur(Number(item.expectedPayout)+carryTotal);
        r.append(l,v);card.appendChild(r);
      }
      card.appendChild(note('Die Nachzahlung wird hier nur als möglicher Übertrag gezeigt. Sie wird dem regulären Soll dieses Monats nicht zugerechnet und verschwindet, sobald eine tatsächliche Rückrechnung erkannt wurde.','note'));
    }
