...
import {buildPayrollControlHistory,buildPayrollCarryovers,payrollCarryoverRegularSollLabel,payrollCardDisplay,payrollProjectedPayout,payrollControlStatusLabel} from './payroll-control.js';
...
export function renderPayrollControl(){
  ...
  const controls=allControls.slice(0,3);
  ...
  const carryovers=buildPayrollCarryovers({controls:allControls,netByMonth});
  wrap.innerHTML='';
  if(!controls.length){
  ...
  for(const item of controls){
    ...
    const summaryEntries=display.waiting
      ?[['Soll-Brutto',item.expectedGross],['Reguläre Auszahlung',item.expectedPayout]]
      :[['Soll-Brutto',item.expectedGross],['Abgerechnet-Brutto',item.actualGross],['Brutto fehlt',item.remainingGross]];
    ...
    if(incoming.length){
      ...
      const projected=payrollProjectedPayout(item,incoming);
      if(projected!=null){
        const r=document.createElement('div');r.className='row';
