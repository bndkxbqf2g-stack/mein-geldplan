...
export function calculateCurrentCycleBudget({giro,transactions,savings,today=new Date(),anchorDate=""}={}){
  ...
  const first=budgetForDate({giro,today:now,payday,nextPayday});
  ...
  return {...budgetForDate({giro:Math.max(0,giro+withdrawals-reserved),today:now,payday,nextPayday}),active:true,provisional,daysToPayday:daysUntilPayday(now,nextPayday)};
}
