import {getTransactions,saveTransactions,getCash,saveCash,getSavings,getFixedCosts,getPendingSalary,savePendingSalary,clearPendingSalary,getBudgetAnchor,saveBudgetAnchor} from './storage.js';
...
export function calculateCurrentCycleBudget({giro,transactions,savings,today=new Date(),anchorDate=""}={}){
  ...
  if(salary){payday=new Date(`${salary.date}T00:00:00`);nextPayday=nextPaydayFrom(payday);}
  else if(anchorDate){payday=new Date(`${String(anchorDate).slice(0,10)}T00:00:00`);nextPayday=plannedPaydayForDate(payday);provisional=true;}
...
export function createBudgetUi({refresh=()=>{}}={}){
  ...
  function processPendingSalary(now=new Date()){
    ...
    const result=applyPendingSalary({pending,transactions:txs(),fixedCosts:getFixedCosts(),today:now});
    if(result.alreadyPosted){clearPendingSalary();return false;}
  ...
  function renderPending(){
    ...
    const due=new Date(`${pending.payoutDate}T00:00:00`),projection=projectPendingSalary({pending,giro:g,cash:c,fixedCosts:fix});
  ...
  function addSalary(){if(getPendingSalary())return notify('Es ist bereits ein Lohn vorgemerkt. Lösche die Vormerkung zuerst, wenn du den Lohn manuell buchen möchtest.',{type:'error',timeout:4500});const value=num('salaryAmount');if(value<=0)return notify('Bitte den Nettolohn eingeben.',{type:'error'});const cycle=cycleKey(new Date()),fix=totalFixedCosts(getFixedCosts());book(value,$('salaryText').value.trim()||'Lohn','salary',{cycle});if(fix>0&&!hasFixedCostForCycle(txs(),cycle))book(-fix,'Fixkosten automatisch abgezogen','fixedcost',{cycle});$('salaryAmount').value='';$('salaryText').value='';refresh();}
