...
export function buildPayrollNetBreakdown({forecast,actual,variableRows=[],retro=[],estimatedNetImpact=null}={}){
  ...
  const taxFreeGross=sum(openRows.filter(row=>row.tax==='steuerfrei').map(row=>row.open));
  const taxableGross=sum(openRows.filter(row=>row.tax==='steuerpflichtig').map(row=>row.open));
  const timeGross=sum(openRows.filter(row=>TIME_KEYS.has(row.key)).map(row=>row.open));
  const shiftGross=sum(openRows.filter(row=>row.key==='shift').map(row=>row.open));
  ...
  const effects=forecast?.netEffects||null;
  ...
  if(allVariableOpen&&effects?.complete){
    ...
    shiftNet=Number.isFinite(Number(effects.shiftAfterTimeNet))?money(effects.shiftAfterTimeNet):null;
    source='detailed';
  }else if(allVariableOpen&&estimatedNetImpact!=null&&!hasUnverified){
