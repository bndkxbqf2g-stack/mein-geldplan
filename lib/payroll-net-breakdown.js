...
import {garnishment2026,vblEmployeeContribution,reportComponents} from './salary.js';
import {storedLegacySummaryReport} from './salary-net-effects.js';
...
function storedActualFallbackEffects(forecast={},actual={},config=SALARY_2026){
  ...
  if(![legalNet,actualLegalNet,actualPayout,actualVbl].every(Number.isFinite))return null;
  const report=storedLegacySummaryReport(forecast,config);
  if(!report)return null;
  const c=reportComponents(report,config);
  const taxableGross=money(c.taxableExtra);
  const taxFreeGross=money(c.taxFreePay);
  if(taxableGross<=0.005)return null;
  ...
  const scenario=(eligibleGross,protectedGross=0)=>{
    ...
    const legalDelta=money(protectedPay+taxableLegalDelta*(eligible/taxableGross));
    ...
    return {legalDelta,legalNet:scenarioLegalNet,vbl:scenarioVbl,garnishable,garnishment:garnish,payout,net:money(payout-actualPayout)};
  };

  const full=scenario(taxableGross,taxFreeGross);
  const time=scenario(c.pay.saturday,taxFreeGross);
  const shift=scenario(c.pay.shift,0);
  ...
  return {
    ...
    timeGross:money(c.pay.night+c.pay.saturday+c.pay.sunday+c.pay.holiday),
    timeNet:time.net,
    shiftGross:money(c.pay.shift),
    shiftType:c.shift||report.inferredShiftType||'none',
