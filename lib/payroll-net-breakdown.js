...
export function buildPayrollNetBreakdown({forecast,actual,variableRows=[],retro=[],estimatedNetImpact=null}={}){
  ...
  const storedActualEffectsCurrent=Number(forecast?.actualNetEffectsVersion)>=4?storedActualEffects:null;
  const effects=storedActualEffectsCurrent||fallbackEffects||storedActualEffects||forecast?.netEffects||null;
  ...
  }else if(estimatedNetImpact!=null&&!retro.length&&!hasUnverified&&(allVariableOpen||!variableRows.length)){
