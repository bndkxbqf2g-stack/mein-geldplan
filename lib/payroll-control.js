...
function retroForMonth(payslips,month){
  ...
  for(const payslip of payslips||[]){
    for(const retro of payslip?.retroPeriods||[]){
      if(samePayrollMonth(retro?.month,month))hits.push({...retro,paidWithMonth:payslip.month,combinedNetAdjustment:payslip.priorAdjustment||0,retroCount:(payslip.retroPeriods||[]).length});
    }
  }
  return hits;
}
...
export function buildPayrollControl({forecast,actual,payslips=[]}={}){
  ...
  const retro=retroForMonth(payslips,forecast.reportMonth||forecast.payoutMonth);
