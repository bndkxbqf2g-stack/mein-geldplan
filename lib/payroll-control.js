...
export function buildPayrollControl({forecast,actual,payslips=[]}={}){
  ...
  const retroGross=sum(retro.map(r=>Math.max(0,Number(r.totalGross)||0)));
  const initialShortfall=expectedGross!=null&&actualGross!=null?money(Math.max(0,expectedGross-actualGross)):null;
  ...
  const variableRows=expectedRows.map(row=>{
    ...
    const open=real==null?null:money(Math.max(0,row.expected-real-retroPaid));
    const status=open==null?'unknown':open<=1?'ok':retroPaid>0?'partial':'open';
    return {...row,actual:real,retro:retroPaid,open,status};
  });
  ...
  let status='waiting';
  if(actual){
    if(remainingGross!=null&&remainingGross<=1&&unexplainedOverCorrection<=1)status=retroGross>0?'settled':'ok';
