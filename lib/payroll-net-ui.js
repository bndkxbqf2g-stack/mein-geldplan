...
export function appendPayrollNetBreakdown(card,item){
  ...
  if(data.actualLegalNet!=null&&data.correctedLegalNet!=null){
    card.appendChild(row(
      'Gesetzliches Netto',
      eur(data.actualLegalNet)+' → '+eur(data.correctedLegalNet),
      {small:'Nach Lohnsteuer und Sozialversicherung, noch vor VBL und Pfändung.'}
    ));
  }

  if(data.taxableGross>0&&data.taxAndSvReduction!=null&&data.vblDelta!=null&&data.garnishmentDelta!=null&&data.taxableNet!=null){
    card.appendChild(row(
      'Rechenweg steuerpflichtiger Teil',
      eur(data.taxableGross)+' − '+eur(data.taxAndSvReduction)+' − '+eur(data.vblDelta)+' − '+eur(data.garnishmentDelta)+' = '+eur(data.taxableNet),
      {small:'steuerpflichtiges Brutto − Steuer/SV − zusätzliche VBL − zusätzliche Pfändung = Nettoeffekt'}
    ));
  }
