...
export function appendPayrollNetBreakdown(card,item){
  ...
  if(data.shiftGross>0){
    const shiftLabel=item.variableRows?.find(entry=>entry.key==='shift')?.label||((item.forecast?.components?.shiftType==='wechsel')?'Wechselschichtzulage':'Schichtzulage');
    card.appendChild(row(
      shiftLabel,
      eur(data.shiftGross)+' brutto → '+(data.shiftNet==null?'Netto noch nicht berechenbar':eur(data.shiftNet)+' netto')
    ));
  }
  ...
  if(data.source==='legacy-summary-split'){
    card.appendChild(note(
      'Steuerfrei und steuerpflichtig wurden aus den gespeicherten Netto-, Pfändungs- und Bruttowerten rekonstruiert. Für die Aufteilung auf einzelne Lohnarten wie Nacht, Sonntag, Samstag oder Schichtzulage müsste der alte Zeitnachweis erneut eingelesen werden.',
      'note payroll-alert'
    ));
