...
export function appendPayrollNetBreakdown(card,item){
  ...
  if(data.shiftGross>0){
    ...
    card.appendChild(row(
      ...
      {small:data.source==='actual-payslip-detailed'?'Differenz: aktuelle Bezügemitteilung → aktuelle Bezügemitteilung + nur diese Zulage':''}
    ));
  }
  ...
  if(data.source==='actual-payslip-detailed'){
