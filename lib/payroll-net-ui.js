...
export function appendPayrollNetBreakdown(card,item){
  ...
  if(!card||!data||(data.taxFreeGross<=0&&data.taxableGross<=0))return;

  const heading=document.createElement('div');
  heading.className='payroll-control-group';
  heading.textContent='Netto der offenen Rückrechnung';
  ...
  for(const [label,value] of [
    ...
    ['Nachzahlung netto',data.totalNet]
  ]){
  ...
  card.appendChild(row(
    ...
    eur(data.taxableGross)+' brutto → '+(data.taxableNet==null?'nach neuem Einlesen berechenbar':eur(data.taxableNet)+' netto')
  ));
  ...
  if(data.correctedPayout!=null){
    const corrected=row(
      'Auszahlung bei vollständiger Korrektur',
      eur(data.correctedPayout),
      {strong:true,small:'Basis: aktuelle Bezügemitteilung'}
    );
    corrected.lastChild.classList.add('good');
    card.appendChild(corrected);
  }

  if(data.source==='legacy-total'){
    ...
    card.appendChild(note(
      'Netto-Prognose aus BMF-Lohnsteuer, Sozialversicherung, VBL und Pfändung. Die spätere Rückrechnung der Bezügemitteilung bleibt der endgültige Ist-Nachweis.'
    ));
  }
}
