...
function renderReportDetails(report){
  ...
  details.innerHTML=rows.length?rows.map(item=>`<div class="row wage-row"><span>${item.code} · ${wageCodeLabel(item.code)}</span><span class="v wage-unit">${wageQuantityLabel(item)}</span></div>`).join(''):'<div class="empty">Keine Zeitlohnarten erkannt.</div>';
}
