...
import { calculateActualBasedForecastNetEffects } from './salary-net-effects.js';
...
function storePayslip(actual){
  ...
  const list=getPayslips().filter(item=>item?.month!==actual.month);
  list.push(actual);
...
async function backfillActualNetEffects(){
  ...
  if(changed){
    if(!saveSalaryForecasts(forecasts))throw new Error('Netto-Rückrechnung konnte nicht dauerhaft gespeichert werden.');
    forecastChanged();
  }
}

async function importPayslips(){
  const input=$('payslipFiles'),status=$('payslipStatus');
  ...
  const results=[];
  try{
    const {parsePayslipDocument}=await import('./payslip.js');
    for(const file of files){
      try{
        ...
        if(!actual?.month){results.push({file:file.name,error:'Abrechnungsmonat konnte nicht sicher erkannt werden.'});continue;}
        storePayslip(actual);
        results.push({file:file.name,actual});
      }catch(error){results.push({file:file.name,error:error?.message||'Bezügemitteilung konnte nicht ausgewertet werden.'});}
    }
    await backfillActualNetEffects();
    refreshPayrollLearning();
    renderPayrollControl();
    forecastChanged();
    const valid=results.filter(result=>result.actual);
    const errors=results.filter(result=>result.error);
    status.textContent=!valid.length
      ?'Keine Bezügemitteilung konnte sicher ausgewertet werden.'
      :errors.length
        ?`${valid.length} Bezügemitteilung(en) gespeichert · ${errors.length} bitte prüfen.`
        :`${valid.length} Bezügemitteilung(en) gespeichert und mit den Prognosen abgeglichen.`;
    if(valid.length)notify('Bezügemitteilung wurde mit der Gehaltsprognose abgeglichen.',{type:'success'});
  }catch(error){
    status.textContent='Bezügemitteilung konnte nicht ausgewertet werden.';
    notify(error?.message||'Bezügemitteilung konnte nicht ausgewertet werden.',{type:'error'});
  }
}
...
export function renderForecastHistory(){
  ...
  const list=getSalaryForecasts().slice().sort((a,b)=>String(a.payoutMonth).localeCompare(String(b.payoutMonth))).reverse().slice(0,3);
...
export function initSalaryUi({onForecastChange}={}){
  ...
  renderForecastHistory();renderPayrollControl();void renderSalaryForecast();
  if(activeReport)renderReportDetails(activeReport);
  void (async()=>{try{await backfillActualNetEffects();refreshPayrollLearning();renderPayrollControl();}catch(error){console.error('[salary-control-init]',error);}})();
