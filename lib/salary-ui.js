...
async function importTimeReports(){
  ...
  try{
    ...
    for(const entry of validReports){try{const f=await calculateSalaryForecast(entry.report);storeForecast(entry.report,f);}catch(error){console.error('[salary-history]',entry.file,error);}}
