...
export async function renderSalaryForecast(){
  ...
  if(!activeReport&&activeForecast){
    const c=activeForecast.components||{},p=c.pay||{},h=c.hours||{};
    ...
    set('pSpringIn',eur(p.springIn));set('pSaturday',eur(p.saturday));set('pTaxFree',eur(activeForecast.protectedPay));
    ...
    set('pShiftSummary',`${Number(h.night||0).toFixed(2)} h Nacht · ${Number(h.saturday||0).toFixed(2)} h Samstag · ${Number(h.sunday||0).toFixed(2)} h Sonntag · ${c.shift==='wechsel'?'Wechselschicht':c.shift==='schicht'?'Schicht':'gespeicherte Prognose'} · Einzelbestandteile bitte neu einlesen`);
    return;
  }
