...
function storeForecast(report,f){
  ...
  list.push({payoutMonth:report.payoutMonth,reportMonth:report.month?`${report.month.year}-${String(report.month.month).padStart(2,'0')}`:null,totalGross:f.totalGross,legalNet:f.legalNet,garnishableNet:f.garnishableNet,garnishment:f.garnishment,payout:f.payout,needsReview:f.needsReview,springIn:f.components.springIn});
...
async function importTimeReports(){
  ...
  try{
    ...
    if(activeReport){if($('rMonth')&&activeReport.month)$('rMonth').textContent=`${String(activeReport.month.month).padStart(2,'0')}/${activeReport.month.year}`;if($('rPayoutMonth'))$('rPayoutMonth').textContent=activeReport.payoutMonth||'–';await renderSalaryForecast();try{const f=await calculateSalaryForecast(activeReport);storeForecast(activeReport,f);renderForecastHistory();forecastChanged();}catch(error){console.error('[salary-history]',error);}}
...
async function applySpringIn(){
  ...
  try{const f=await calculateSalaryForecast(activeReport);storeForecast(activeReport,f);renderForecastHistory();forecastChanged();}catch(error){console.error('[spring-in]',error);}
