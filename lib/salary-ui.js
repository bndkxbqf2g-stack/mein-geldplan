...
import {samePayrollMonth,payrollMonthKey} from './payroll-month.js';
...
export function restoreReportFromForecast(forecast){
  ...
  return {month:{year,month},payoutMonth:String(forecast.payoutMonth),items:forecast.reportItems.map(item=>({...item})),unknownCodes:[],needsReview:Boolean(forecast.needsReview),springIn:forecast.springIn||{duties:0,hours:0}};
}
...
function refreshPayrollLearning(){
  ...
  let history=getPayrollLearning();
  for(const forecast of forecasts){
    const actual=payslips.find(item=>samePayrollMonth(item?.month,forecast?.payoutMonth));
    if(!actual)continue;
    history=mergePayrollLearning(history,buildPayrollLearningSnapshot({forecast,actual}));
  }
  if(!savePayrollLearning(history))throw new Error('Payroll-Lernhistorie konnte nicht dauerhaft gespeichert werden.');
  return history;
}
...
async function importPayslips(){
  ...
  try{
    const pdf=await import('./pdf.js'),mod=await import('./payslip.js');let last=null,lastComparison=null;
    for(const file of files){const read=await pdf.readPdfText(file);if(read.needsOcr){status.textContent='Kein sicherer PDF-Text erkannt – bitte prüfen.';continue;}const actual=mod.parsePayslipDocument(read.text);storePayslip(actual);const currentForecasts=getSalaryForecasts();const forecast=currentForecasts.find(f=>samePayrollMonth(f?.payoutMonth,actual?.month));last=actual;lastComparison=forecast?mod.comparePayslip(forecast,actual):{status:'review',rows:[],maxAbsDiff:null};}
...
export async function renderSalaryForecast(){
  if($('pBasePay'))$('pBasePay').textContent=eur(SALARY_2026.fixed.basePay);if($('springInHourlyRate'))$('springInHourlyRate').textContent=eur(springInHourlyRate());if($('pCareAllowance'))$('pCareAllowance').textContent=eur(SALARY_2026.fixed.careAllowance);if($('pUniversityAllowance'))$('pUniversityAllowance').textContent=eur(SALARY_2026.fixed.universityAllowance);
  const set=(id,v)=>{if($(id))$(id).textContent=v;};
  if(!activeReport&&activeForecast){
    const c=activeForecast.components||{},h=c.hours||{};
    set('pBrutto',eur(activeForecast.totalGross));set('pNettoBasis',eur(activeForecast.legalNet));set('pLegalNet',eur(activeForecast.legalNet));
    const fixedTotal=SALARY_2026.fixed.basePay+SALARY_2026.fixed.careAllowance+SALARY_2026.fixed.universityAllowance;
    set('pProtected',eur(activeForecast.protectedPay));set('pSurcharges',eur((activeForecast.totalGross||0)-fixedTotal));
    set('pPfNetto',eur(activeForecast.garnishableNet));set('pGarnish',eur(activeForecast.garnishment));set('pPayout',eur(activeForecast.payout));set('pPayoutDetail',eur(activeForecast.payout));
    set('pVariableNet',eur((activeForecast.taxableGross||0)-fixedTotal));
    set('pSpringIn',eur(c.springIn));set('pSaturday',eur(c.saturday));set('pTaxFree',eur(activeForecast.protectedPay));
    set('pTax',eur((activeForecast.wageTax||0)+(activeForecast.churchTax||0)+(activeForecast.solidarity||0)));
    set('pSocial',eur((activeForecast.health||0)+(activeForecast.care||0)+(activeForecast.pension||0)+(activeForecast.unemployment||0)));set('pVbl',eur(activeForecast.vbl));
    const hours=Object.keys(h).length?`${Number(h.night||0).toFixed(2)} h Nacht · ${Number(h.saturday||0).toFixed(2)} h Samstag · ${Number(h.sunday||0).toFixed(2)} h Sonntag · `:'Stunden nicht gespeichert · ';
    set('pShiftSummary',`${hours}${c.shiftType==='wechsel'?'Wechselschicht':c.shiftType==='schicht'?'Schicht':'gespeicherte Prognose'} · Einzelbestandteile bitte neu einlesen`);
    return;
  }
  if(!activeReport)return;
  try{
    const f=await calculateSalaryForecast(activeReport);
...
async function importTimeReports(){
  ...
  try{
    ...
    if(activeReport){activeForecast=getSalaryForecasts().find(item=>item.payoutMonth===activeReport.payoutMonth)||null;if($('rMonth')&&activeReport.month)$('rMonth').textContent=`${String(activeReport.month.month).padStart(2,'0')}/${activeReport.month.year}`;if($('rPayoutMonth'))$('rPayoutMonth').textContent=activeReport.payoutMonth||'–';await renderSalaryForecast();}
    await backfillActualNetEffects();
    refreshPayrollLearning();
...
async function backfillActualNetEffects(){
  ...
  let changed=false;
  for(const forecast of forecasts){
    const actual=payslips.find(entry=>samePayrollMonth(entry?.month,forecast?.payoutMonth));
    if(!actual||forecast?.actualNetEffectsVersion===5)continue;
    try{
      const effects=await calculateActualBasedForecastNetEffects(forecast,actual,SALARY_2026);
      if(effects){
        forecast.actualNetEffects=effects;
        forecast.actualNetEffectsVersion=5;
        changed=true;
      }
...
export function initSalaryUi({onForecastChange}={}){
  ...
  const saved=getSalaryForecasts().slice().sort((a,b)=>String(a.payoutMonth).localeCompare(String(b.payoutMonth))).at(-1);
  activeForecast=saved||null;
  activeReport=restoreReportFromForecast(saved);
  if(activeReport){if($('rMonth'))$('rMonth').textContent=`${String(activeReport.month.month).padStart(2,'0')}/${activeReport.month.year}`;if($('rPayoutMonth'))$('rPayoutMonth').textContent=salaryMonthLabel(saved?.payoutMonth||activeReport.payoutMonth);}
  else if(saved){if($('rMonth'))$('rMonth').textContent=salaryMonthLabel(payrollMonthKey(saved.reportMonth)||saved.reportMonth);if($('rPayoutMonth'))$('rPayoutMonth').textContent=salaryMonthLabel(payrollMonthKey(saved.payoutMonth)||saved.payoutMonth);}
  else if(saved){if($('rMonth'))$('rMonth').textContent=salaryMonthLabel(saved.reportMonth);if($('rPayoutMonth'))$('rPayoutMonth').textContent=salaryMonthLabel(saved.payoutMonth);}
  renderSalaryForecast();renderForecastHistory();renderPayrollControl();void (async()=>{await backfillStoredNetEffects();await backfillActualNetEffects();refreshPayrollLearning();})();
