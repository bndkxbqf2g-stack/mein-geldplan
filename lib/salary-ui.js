...
async function importTimeReports(){
  ...
  try{
    ...
    if(details)details.innerHTML=reports.flatMap(r=>r.report?r.report.items.concat(r.report.unknownCodes):[]).map(i=>`<div class="row"><span>${i.code} · ${i.type||'Unbekannt'}</span><span class="v">${i.hours==null?'Bitte prüfen':`${i.hours.toFixed(2)} ${(i.code==='5161'||i.code==='5162')?'Tg.':'h'}`}</span></div>`).join('')||'<div class="empty">Keine Zeitlohnarten erkannt.</div>';
...
async function applySpringIn(){
  ...
  try{const f=await calculateSalaryForecast(activeReport);storeForecast(activeReport,f);renderForecastHistory();}catch(error){console.error('[spring-in]',error);}
