...
function refreshPayrollLearning(){
  ...
  for(const forecast of forecasts){
    const actual=payslips.find(item=>item?.month===forecast?.payoutMonth);
    if(!actual)continue;
...
export function initSalaryUi({onForecastChange}={}){
  ...
  if(activeReport){if($('rMonth'))$('rMonth').textContent=`${String(activeReport.month.month).padStart(2,'0')}/${activeReport.month.year}`;if($('rPayoutMonth'))$('rPayoutMonth').textContent=activeReport.payoutMonth;}
