...
import {renderPayrollControl} from './payroll-control-ui.js';
...
function storeForecast(report,f,baseline=null){
...
async function importTimeReports(){
  ...
  try{
    ...
    for(const entry of validReports){try{const f=await calculateSalaryForecast(entry.report);storeForecast(entry.report,f,baseline);}catch(error){console.error('[salary-history]',entry.file,error);}}
...
async function applySpringIn(){
  ...
  try{const f=await calculateSalaryForecast(activeReport),baseline=await calculateSalaryForecast({items:[],unknownCodes:[],needsReview:false});storeForecast(activeReport,f,baseline);renderForecastHistory();renderPayrollControl();forecastChanged();}catch(error){console.error('[spring-in]',error);}
...
export function initSalaryUi({onForecastChange}={}){
  ...
  renderSalaryForecast();renderForecastHistory();renderPayrollControl();
