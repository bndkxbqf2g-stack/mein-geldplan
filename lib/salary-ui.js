...
import {calculateNetEffects,calculateStoredForecastNetEffects} from './salary-net-effects.js';
...
async function backfillStoredNetEffects(){
  ...
  for(const forecast of list){
    if(forecast?.netEffects||!forecast?.components)continue;
    try{
      const netEffects=await calculateStoredForecastNetEffects(forecast,SALARY_2026);
      if(netEffects){forecast.netEffects=netEffects;changed=true;}
...
export function initSalaryUi({onForecastChange}={}){
  ...
  renderSalaryForecast();renderForecastHistory();renderPayrollControl();void backfillStoredNetEffects();
