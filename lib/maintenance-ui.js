import {getTransactions,saveTransactions,getCash,saveCash,getSavings,saveSavings,getSalaryForecasts,saveSalaryForecasts,getPayslips,savePayslips,getPayrollLearning,savePayrollLearning,getFixedCosts,saveFixedCosts,getFixedCostOverrides,saveFixedCostOverrides,getPendingSalary,savePendingSalary,clearPendingSalary,getBudgetAnchor,saveBudgetAnchor,getTheme,saveTheme,getShowExplanations,saveShowExplanations,migrateStorage,DATA_SCHEMA_VERSION,storageKeys} from './storage.js';
import {APP_VERSION} from '../config/version.js';
import {$,notify,setButtonBusy} from './ui.js';

const VERSION=7;
function snapshot(){return {version:VERSION,schemaVersion:DATA_SCHEMA_VERSION,appVersion:APP_VERSION,exportedAt:new Date().toISOString(),transactions:getTransactions(),cash:getCash(),savings:getSavings(),salaryForecasts:getSalaryForecasts(),payslips:getPayslips(),payrollLearning:getPayrollLearning(),fixedCosts:getFixedCosts(),fixedCostOverrides:getFixedCostOverrides(),pendingSalary:getPendingSalary(),budgetAnchor:getBudgetAnchor(),theme:getTheme(),showExplanations:getShowExplanations()};}
function normalizeBackup(data){
  ...
  return {version,transactions:Array.isArray(data.transactions)?data.transactions:[],cash:Number.isFinite(Number(data.cash))?Math.max(0,Number(data.cash)):0,savings:data.savings&&typeof data.savings==='object'?data.savings:{positions:[],allocations:[]},salaryForecasts:Array.isArray(data.salaryForecasts)?data.salaryForecasts:[],payslips:Array.isArray(data.payslips)?data.payslips:[],payrollLearning:Array.isArray(data.payrollLearning)?data.payrollLearning:[],fixedCosts:'fixedCosts'in data?(data.fixedCosts===null?null:(Array.isArray(data.fixedCosts)?data.fixedCosts:[])):null,fixedCostOverrides:Array.isArray(data.fixedCostOverrides)?data.fixedCostOverrides:[],pendingSalary:data.pendingSalary&&typeof data.pendingSalary==='object'?data.pendingSalary:null,budgetAnchor:typeof data.budgetAnchor==='string'?data.budgetAnchor:'',theme:['system','light','dark'].includes(data.theme)?data.theme:'system',showExplanations:data.showExplanations!==false};
}
function restore(data){
  const clean=normalizeBackup(data),store=typeof localStorage!=='undefined'?localStorage:null,previous=store?snapshot():null;
  try{
    if(!saveTransactions(clean.transactions)||!saveCash(clean.cash)||!saveSavings(clean.savings)||!saveSalaryForecasts(clean.salaryForecasts)||!savePayslips(clean.payslips)||!savePayrollLearning(clean.payrollLearning))throw new Error('Speichern fehlgeschlagen.');
    if(clean.fixedCosts!==null&&!saveFixedCosts(clean.fixedCosts))throw new Error('Fixkosten konnten nicht gespeichert werden.');
    if(!saveFixedCostOverrides(clean.fixedCostOverrides))throw new Error('Fixkosten-Ausnahmen konnten nicht gespeichert werden.');
