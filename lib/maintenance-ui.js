import {getTransactions,saveTransactions,getCash,saveCash,getSavings,saveSavings,getSalaryForecasts,saveSalaryForecasts,getPayslips,savePayslips,getFixedCosts,saveFixedCosts} from './storage.js';
import {$} from './ui.js';

const VERSION=1;
function snapshot(){return {version:VERSION,exportedAt:new Date().toISOString(),transactions:getTransactions(),cash:getCash(),savings:getSavings(),salaryForecasts:getSalaryForecasts(),payslips:getPayslips(),fixedCosts:getFixedCosts()};}
function restore(data){if(!data||typeof data!=='object')throw new Error('Ungültige Sicherungsdatei.');saveTransactions(data.transactions||[]);saveCash(data.cash||0);saveSavings(data.savings||{});saveSalaryForecasts(data.salaryForecasts||[]);savePayslips(data.payslips||[]);if('fixedCosts'in data)saveFixedCosts(data.fixedCosts||[]);}
