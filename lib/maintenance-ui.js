...
function normalizeBackup(data){
  ...
  return {version,transactions:Array.isArray(data.transactions)?data.transactions:[],cash:Number.isFinite(Number(data.cash))?Math.max(0,Number(data.cash)):0,savings:data.savings&&typeof data.savings==='object'?data.savings:{positions:[],allocations:[]},salaryForecasts:Array.isArray(data.salaryForecasts)?data.salaryForecasts:[],payslips:Array.isArray(data.payslips)?data.payslips:[],fixedCosts:'fixedCosts'in data?(Array.isArray(data.fixedCosts)?data.fixedCosts:[]):null,pendingSalary:data.pendingSalary&&typeof data.pendingSalary==='object'?data.pendingSalary:null,budgetAnchor:typeof data.budgetAnchor==='string'?data.budgetAnchor:'',theme:['system','light','dark'].includes(data.theme)?data.theme:'system'};
}
function restore(data){
  ...
  try{
    ...
    saveTheme(clean.theme);migrateStorage();return true;
  }catch(error){
