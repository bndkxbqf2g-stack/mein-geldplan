import {getTransactions,getSavings,getSalaryForecasts} from './storage.js';
...
export function expectedSalarySlots({today=new Date(),transactions=[],forecasts=[]}={}){
  ...
  return months.map(payoutMonth=>{
    ...
    return {
      ...
      status:forecast?'forecast':'outstanding',
      payout:forecast&&Number.isFinite(Number(forecast.payout))?Number(forecast.payout):null,
...
export function createHistoryUi({budgetUi}={}){
  function renderExpectedSalaries(){
    ...
    const slots=expectedSalarySlots({today:new Date(),transactions:getTransactions(),forecasts:getSalaryForecasts()});
    wrap.innerHTML='';
    slots.forEach(slot=>{
      ...
      const detail=document.createElement('span');detail.textContent=slot.status==='forecast'
        ?`Zeitnachweis ${slot.reportMonth?monthLabel(slot.reportMonth):'vorhanden'}${slot.needsReview?' · Bitte prüfen':''}`
      ...
      value.textContent=slot.status==='forecast'?eur(slot.payout):'Ausstehend';
