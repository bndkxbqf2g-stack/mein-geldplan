...
export function createHistoryUi({budgetUi}={}){
  function renderOverview(){const b=budgetUi.getBudget(),g=budgetUi.getGiro(),c=budgetUi.getCash(),tx=getTransactions(),items=tx.filter(relevant),available=g+c;
    const vals={ovGiro:eur(g),ovCash:eur(c),ovAvailable:eur(available),ovNextPay:`Nächster Lohn ${fmt(b.nextPayday)}`,ovPayDays:b.remainingDays,ovDay:eur(b.dailyBudget),ovWeek:eur(b.weeklyBudget),ovFix:eur(totalFixedCosts(getFixedCosts())),ovCashKpi:eur(c),ovSalaryCount:tx.filter(t=>t.type==='salary').length};Object.entries(vals).forEach(([id,v])=>{if($(id))$(id).textContent=v;});
