...
function calcSparen(){
  var uStand=num("uStand"),uOwn=num("uOwn"),uSteffi=num("uSteffi"),uExtra=num("uExtra"),uMonths=Math.max(0,num("uMonths")),uGoal=Math.max(0,num("uGoal"));
  var uMonth=uOwn+uSteffi+uExtra;
  var uForecast=uStand+(uMonth*uMonths);
  var uMissing=Math.max(0,uGoal-uForecast);
  if($("uMonth"))$("uMonth").textContent=eur(uMonth);
  if($("uForecast"))$("uForecast").textContent=eur(uForecast);
  if($("uMissing"))$("uMissing").textContent=eur(uMissing);

  var aStand=num("aStand"),aRate=num("aRate"),aMonths=Math.max(0,num("aMonths"));
  var aForecast=aStand+(aRate*aMonths);
  if($("aForecast"))$("aForecast").textContent=eur(aForecast);
}
