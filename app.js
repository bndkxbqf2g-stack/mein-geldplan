...
function updateBudget(){
  var days=cycleDays(),giro=currentGiro(),cash=num("bCarryCash"),available=giro+cash,day=available/days,week=day*7;
  ...
  $("sTotal").textContent=eur(available);$("sGiro").textContent=eur(giro);
  var after=giro-num("sWithdrawAmount");$("sAfter").textContent=eur(after);
}
function renderTx(){
...
function addIncome(){
  ...
  var a=txs();a.push({id:Date.now(),type:"income",amount:v,date:dateKey(new Date()),text:$("giroText").value.trim()||"Zahlungseingang"});saveTx(a);$("giroIncome").value="";$("giroText").value="";refresh();
}
function addExpense(){
  var v=num("giroExpense");if(v<=0){alert("Bitte einen positiven Ausgabebetrag eingeben.");return;}
  var a=txs();a.push({id:Date.now(),type:"expense",amount:-v,date:dateKey(new Date()),text:$("giroText").value.trim()||"Ausgabe"});saveTx(a);$("giroExpense").value="";$("giroText").value="";refresh();
}
function withdraw(){
  ...
  var a=txs();a.push({id:Date.now(),type:"withdrawal",amount:-v,date:dateKey(new Date()),text:"Bargeldabhebung"});saveTx(a);$("sWithdrawAmount").value="";refresh();
}
function refresh(){renderTx();updateBudget();if($("giroCurrent"))$("giroCurrent").textContent=eur(currentGiro());}
function init(){
  ensureBase();
