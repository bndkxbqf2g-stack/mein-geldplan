...
function lastBankDay(y,m){var d=new Date(y,m+1,0);while(d.getMonth()===m&&!isBankDay(d))d.setDate(d.getDate()-1);return d;}
function nextPayDate(){var now=new Date(),y=now.getFullYear(),m=now.getMonth(),d=lastBankDay(y,m);d.setHours(20,30,0,0);if(now>d){m++;if(m>11){m=0;y++;}d=lastBankDay(y,m);d.setHours(20,30,0,0);}return d;}
function cycleDays(){var now=new Date(),n=nextPayDate(),today=new Date(now.getFullYear(),now.getMonth(),now.getDate()),nd=new Date(n.getFullYear(),n.getMonth(),n.getDate());return Math.max(1,Math.round((nd-today)/86400000));}
...
function daysUntilNextSunday(){var now=new Date(),today=new Date(now.getFullYear(),now.getMonth(),now.getDate()),delta=(7-today.getDay())%7;return delta===0?7:delta;}
...
function latestSalaryDate(){var a=txs().filter(function(t){return t.type==="salary";});if(!a.length)return null;return a[a.length-1].date||null;}
function isSalaryDay(){var sd=latestSalaryDate();return !!sd && sd===dateKey(new Date());}
function remainingPayDays(){var now=new Date(),n=nextPayDate(),today=new Date(now.getFullYear(),now.getMonth(),now.getDate()),nd=new Date(n.getFullYear(),n.getMonth(),n.getDate());return Math.max(1,Math.round((nd-today)/86400000));}
function withdrawalDays(){var last=lastWithdrawalDate(),today=dateKey(new Date());if(last===today)return daysUntilNextSunday();return daysUntilNextSunday();}
function firstWithdrawalSuggested(giro){var payDays=remainingPayDays();var daysToSun=daysUntilNextSunday();return payDays>0?Math.max(0,giro/payDays*daysToSun):0;}
...
function updateBudget(){
  ...
  var untilWithdrawal=daysUntilNextSunday();
  var basis=giro;
  ...
  if($("mainDays"))$("mainDays").textContent=untilWithdrawal;
  var day=(payDays>0?basis/payDays:0),week=day*7;
  ...
  if($("budgetNote")){
    if(weekly){var wd=lastWithdrawalDate();$("budgetNote").textContent="Abhebung am "+wd.split("-").reverse().join(".")+" gestartet. Das Bargeld ist für 7 Tage bis zur nächsten Sonntagsabhebung gedacht. Tagessatz und Wochensatz basieren weiterhin nur auf dem aktuellen Girokonto und den verbleibenden Tagen bis zum nächsten Lohn.";}
    else{$("budgetNote").textContent="Der erste Abhebungsbetrag am Lohntag wird anteilig bis zum folgenden Sonntag berechnet. Danach wird sonntags jeweils für bis zu 7 Tage abgehoben.";}
  }
}
function renderTx(){
...
function addSalary(){
  ...
  try{localStorage.setItem("meinGeldplanMonthlyFix",String(fix));}catch(e){}
...
function sunday(){
  var konto=currentGiro(),cash=cashBalance();
  if($("bCarryCash")){if(document.activeElement!==$("bCarryCash"))$("bCarryCash").value=cash.toFixed(2);else{var typed=num("bCarryCash");if(Math.abs(typed-cash)>0.009){saveCash(typed);cash=Math.max(0,typed);}}}
  if($("sTotal"))$("sTotal").textContent=eur(konto+cash);
  if($("sGiro"))$("sGiro").textContent=eur(konto);
  if($("sCash"))$("sCash").textContent=eur(cash);
  var pay=remainingPayDays(), day=pay>0?konto/pay:0, first=!lastWithdrawalDate() || isSalaryDay();
  var suggested=first?firstWithdrawalSuggested(konto):regularWithdrawalSuggested(konto);
...
function resetApp(){if(!confirm("Wirklich alle gespeicherten Eingaben und Buchungen löschen?"))return;localStorage.removeItem("meinGeldplanGiroTx");localStorage.removeItem("meinGeldplanCash");localStorage.removeItem("meinGeldplanMonthlyFix");location.reload();}
...
function init(){
 ensureBase();
 try{var mf=localStorage.getItem("meinGeldplanMonthlyFix");if(mf!==null && $("monthlyFix"))$("monthlyFix").value=Number(mf).toFixed(2);}catch(e){}
 setDefaultMonth();if($("bCarryCash"))$("bCarryCash").value=cashBalance().toFixed(2);
